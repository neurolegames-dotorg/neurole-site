...
import {
  ...
  getDisorderCategory, askNeuroleAIRaw,
  getSavedDailyCompletion, saveDailyCompletion, getDailyStats,
  submitGlobalDailyResult, getStreakCount
} from '../utils/helpers'
...
export default function DailyGamePage() {
  ...
  useEffect(() => {
    const saved = getSavedDailyCompletion()
    const alreadyCompleted = !!saved
    const streak = getStreakCount()
    if (streak.count >= 3) {
      setStreakLabel(`🔥 ${streak.count}-day streak`)
    } else if (streak.count >= 1) {
      setStreakLabel(`${streak.count}-day streak`)
    } else {
      setStreakLabel('No streak yet — play today to start one!')
    }
    const dateLabel = new Date().toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })
    setStartMeta(dateLabel)
    if (alreadyCompleted) {
      setPlayBtnText('View Result')
    } else {
      setPlayBtnText('Play')
    }
    const caseNum = getDayIndex() + 1
  ...
  const loadCase = useCallback(async () => {
    ...
    if (!chosen || !answer) {
      const fallbackIdx = getDayIndex() % FALLBACK_CASES.length
    ...
    let newState = {
      symptoms, answer, synonyms, explanation,
      shown: 1, attemptsUsed: 0, guesses: [], solved: false, alreadyCompletedToday: alreadyCompleted
    }
  ...
  // The original daily-game.html landing navigates straight to the play page on
  // click (window.location.href = 'daily-game-play.html'), for both "Play" and
  // "View Result"; the play page then restores any saved completion. Mirror that.
  const handlePlay = useCallback(() => {
    navigate('/daily-game-play')
  }, [navigate])
  ...
  const showResult = useCallback((w, gs) => {
    ...
    setWon(w)
    saveDailyCompletion(w, state.attemptsUsed, state.answer, state.explanation)
    setResultModalOpen(true)
    const dateKey = dateStrForDayIndex(getDayIndex())
    submitGlobalDailyResult(dateKey, w ? state.attemptsUsed : null)
  }, [])
...
Keep your answer to 2-4 sentences, plain language, encouraging tone.`
  ...
  const renderDistribution = (dist, label) => {
    ...
    return (
      <div>
        {label && <p id="result-distribution-label" className="rmodal-section-label" style={{ marginTop: 16 }}>{label}</p>}
        {dist.map((count, i) => {
  ...
  return (
    <main className="wrap case-shell">
      {!playing && (
        <div className="start-screen" style={{ position: 'fixed', inset: 0, zIndex: 9999, background: '#FAFAFA', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflowY: 'auto', padding: 'max(60px,env(safe-area-inset-top,60px)) 28px max(40px,env(safe-area-inset-bottom,40px))', textAlign: 'center' }}>
          <Link to="/" className="back-home" style={{ position: 'absolute', top: 18, left: 20, marginBottom: 0 }}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7" /></svg>
            Home
          </Link>
          <div className="start-screen-inner" style={{ maxWidth: 400, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div className="start-logo" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
              <svg width="50" height="50" viewBox="0 0 24 24" fill="none" stroke="#8B2635" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="2,12 5,12 7,5 9,19 11,9 13,15 15,12 22,12" />
              </svg>
            </div>
            <h1 style={{ fontFamily: 'var(--game-font)', fontWeight: 800, letterSpacing: '-.02em', fontSize: 'clamp(34px,8vw,46px)', margin: '0 0 10px' }}>The Daily Case</h1>
            <p style={{ color: 'var(--ink-soft)', fontSize: 15, margin: '0 0 26px', maxWidth: 300, lineHeight: 1.6, fontFamily: 'var(--game-font)' }}>A new patient every day. Read the symptoms and name the diagnosis — 5 tries, then the answer is revealed.</p>
            <div className="start-pills" style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: 18, flexWrap: 'wrap' }}>
              <span className="start-pill pill-red" style={{ fontFamily: 'var(--game-font)', fontSize: 10.5, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', padding: '4px 12px', borderRadius: 20 }}>🩺 Daily reset</span>
              <span className="start-pill pill-purple" style={{ fontFamily: 'var(--game-font)', fontSize: 10.5, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', padding: '4px 12px', borderRadius: 20 }}>🧠 Neurology</span>
              <span className="start-pill pill-green" style={{ fontFamily: 'var(--game-font)', fontSize: 10.5, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', padding: '4px 12px', borderRadius: 20 }}>✓ Free</span>
            </div>
            <div className="start-actions" style={{ display: 'flex', gap: 12, marginBottom: 22, width: '100%' }}>
              <button className="btn ghost" data-signin style={{ flex: 'none', padding: '13px 22px', borderRadius: 12, fontFamily: 'var(--game-font)', fontWeight: 700 }}>Sign in</button>
              <button className="btn" onClick={handlePlay} style={{ flex: 1, borderRadius: 12, fontFamily: 'var(--game-font)', fontWeight: 700 }}>{playBtnText}</button>
            </div>
            <div id="streak-label" style={{ fontFamily: 'var(--game-font)', fontSize: 13, fontWeight: 600, color: 'var(--neuro-blue-deep)', marginTop: 6, minHeight: 22 }}>{streakLabel}</div>
            <div className="start-meta" style={{ fontFamily: 'var(--game-font)', fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.9, marginTop: 4 }}>{startMeta}</div>
            <Link to="/archive" style={{ fontFamily: 'var(--game-font)', fontSize: 12, color: 'var(--ink-soft)', textDecoration: 'none', marginTop: 16, letterSpacing: '.02em' }}>View case archive →</Link>
          </div>
        </div>
      )}

      <div id="game-content" style={{ display: playing ? 'block' : 'none' }}>
        {sheetWarning && (
      ...
      <button className="chat-fab" id="case-chat-fab" aria-label="Explain this case" onClick={() => setChatOpen(true)}
        style={{ position: 'fixed', bottom: 26, right: 26, height: 50, padding: '0 22px 0 18px', borderRadius: 30, background: 'var(--neuro-blue-bright)', color: '#fff', border: 'none', display: playing ? 'flex' : 'none', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 8px 22px rgba(0,0,0,.18)', fontFamily: 'var(--mono)', fontSize: 12.5, letterSpacing: '.06em', textTransform: 'uppercase', zIndex: 60 }}>
      ...
      <div className={`modal-backdrop rmodal-backdrop${resultModalOpen ? ' open' : ''}`} id="result-modal"
        ...
        <div className="rmodal-card" style={{ background: '#fff', maxWidth: 480, width: '100%', borderRadius: 3, overflow: 'hidden', boxShadow: '0 2px 40px rgba(0,0,0,.22)' }}>
          ...
          <div className="rmodal-section" style={{ padding: '16px 20px' }}>
            ...
            {renderDistribution(dailyStats.distribution, 'Guess Distribution — Your History')}
          </div>

          <div className="rmodal-section" style={{ padding: '0 20px 16px', textAlign: 'center' }}>
            <p className="rmodal-countdown" style={{ fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--ink-soft)', marginBottom: 12 }}>{countdownText}</p>
