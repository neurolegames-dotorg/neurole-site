...
export function saveDailyCompletionForIndex(idx, won, attemptsUsed, answer, explanation) {
  try {
    localStorage.setItem(dailyStorageKeyForIndex(idx), JSON.stringify({
      won, attemptsUsed, answer, explanation
    }));
...
export function saveDailyCompletion(won, attemptsUsed, answer, explanation) {
  saveDailyCompletionForIndex(getDayIndex(), won, attemptsUsed, answer, explanation);
}
